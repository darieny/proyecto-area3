...
function AsignarTecnicoModal({ open, visita, onClose, onSaved }) {
  ...
  useEffect(() => {
    if (visita) {
      setTecnicoId(visita.tecnico_asignado_id ?? "");
    } else {
      setTecnicoId("");
    }
  }, [visita]);
...
export default function VisitasPage() {
  ...
  // Leer filtros iniciales SOLO una vez
  const initialFiltersRef = useRef(parseQS(sp));
  ...
  const kpis = useMemo(() => {
    ...
    for (const v of items) {
      ...
      // Atrasadas: PROGRAMADA cuyo fin ya pasó
  ...
  // Sincroniza la URL cuando cambian los filtros
  useEffect(() => {
  ...
  const closeAsignar = () => {
    setVisitaAsignar(null);
    setAsignarOpen(false);
  };
  const onAsignado = async () => {
    closeAsignar();
    await reload?.();
  };
